...
---
---
## Auditoria de Planos e Gating (Março 2026)
- [x] Implementação do `SubscriptionGatingService` no backend para centralizar regras de limites.
- [x] Imposição estrita do limite de 100 pacientes para o plano STARTER/TRIAL no `PatientService`.
- [x] Visual Gating na página de Relatórios: Métricas avançadas borradas para usuários Starter com CTA de upgrade.
- [x] Visual Gating na página de Pacientes: Contador de limite e bloqueio do botão "Novo Paciente" ao atingir 100.
- [x] Novo endpoint `/subscription/me` para consulta de status e plano pelo frontend.
- [x] Tratamento de erro global `SubscriptionLimitException` (HTTP 403) com mensagens amigáveis.
